...
const deleteEvent = async (req, res) => {
  try {
    await pool.query("DELETE FROM Event_Detail WHERE Unique_Id = $1", [
      req.params.id,
    ]);
...
const cancelRegistration = async (req, res) => {
  try {
    await pool.query("DELETE FROM Registrations WHERE Unique_Id = $1", [
      req.params.id,
    ]);
...
const upcomingEvent = async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM Event_Detail WHERE Event_Time > NOW() ORDER BY Event_Time ASC"
    );
    return res.status(200).json({
      success: true,
      data: result.rows,
